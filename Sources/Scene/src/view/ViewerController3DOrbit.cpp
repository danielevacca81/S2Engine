...
bool ViewerController3DOrbit::doDragBegin( const Math::ivec2 &screenPos )
{
	...
	if( cDir.y != 0.0 || cDir.x != 0.0 ) // todo da migliorare
		_startTheta = _lastTheta = Math::degrees( Math::atan( cDir.y, cDir.x ) );
	else
	{
		Math::dvec3 side = viewer()->camera().side();
		_startTheta = _lastTheta = Math::degrees( Math::atan( side.y, side.x ) ) - 90.0;
	}

	_startPhi = _lastPhi = Math::degrees( Math::atan( cDir.z, Math::distance(cDir.x,cDir.y) ) );
