...
// ------------------------------------------------------------------------------------------------
void Frustum::computePlanes( const Math::dmat4 &modelView, const Math::dmat4 &projection )
{
...
Frustum::TestResult Frustum::isInside( const Math::dvec3 &p, double radius ) const
{
	...
	for( int i=0; i < 6; ++i )
	{
		...
		if( dist < -radius )
		{
			return TestResult::FrustumOutside;
		}
		else if( dist < radius )
		{
			result = TestResult::FrustumIntersect;
		}
	}
