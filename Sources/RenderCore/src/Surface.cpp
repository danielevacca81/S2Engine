...
// ------------------------------------------------------------------------------------------------
Surface::Surface( const std::initializer_list<AttachmentDescription> &init )
: _width( 64 )
, _height( 64 )
, _fbo( FrameBuffer::New() )
, _picker( std::make_shared<SurfacePicker>() )
{
	for( auto &i: init )
...
// ------------------------------------------------------------------------------------------------
PickerConstants::Value Surface::pickValueAt( int32_t x, int32_t y )
{
	if( !_picker )
...
// ------------------------------------------------------------------------------------------------
void Surface::draw( const PrimitiveType &primitiveType, const VertexArrayPtr &vao, const DrawState &ds )           const { Renderer::draw( _fbo, primitiveType , vao, sanitizeDrawState( ds, this ) );}
void Surface::draw( const PrimitiveType &primitiveType, const PrimitiveBufferPtr &primitive, const DrawState &ds ) const { Renderer::draw( _fbo, primitiveType, primitive, sanitizeDrawState( ds, this ) ); }
void Surface::draw( const PrimitiveBatch& batch, const DrawState& ds )                                             const { Renderer::draw( _fbo, batch, sanitizeDrawState( ds, this ) );}
...
void Surface::swap( uint32_t targetFBO, const FrameBuffer::AttachmentPoint &att ) const
{
	...
	Renderer::draw( targetFBO, PrimitiveType::TriangleStrip, quad, ds );
}
...
{
	//
	swap( Renderer::defaultFrameBufferObject(), FrameBuffer::ColorAttachment0 );
}
