...
{
	const float a = ( hexValue & 0xFF ) / 255.0f; hexValue >>= 8;
	const float b = ( hexValue & 0xFF ) / 255.0f; hexValue >>= 8;
	const float g = ( hexValue & 0xFF ) / 255.0f; hexValue >>= 8;
	const float r = ( hexValue & 0xFF ) / 255.0f;
	return Color( r, g, b, a );
}

// ------------------------------------------------------------------------------------------------
Color Color::fromHex64( uint64_t hexValue64 )
{
	const float a = ( hexValue64 & 0xFFFF ) / 65535.0f; hexValue64 >>= 16;
	const float b = ( hexValue64 & 0xFFFF ) / 65535.0f; hexValue64 >>= 16;
	const float g = ( hexValue64 & 0xFFFF ) / 65535.0f; hexValue64 >>= 16;
	const float r = ( hexValue64 & 0xFFFF ) / 65535.0f;
	return Color( r, g, b, a );
}
