...
Path         S2ENGINE_API makepath        ( const std::wstring &path );
Path         S2ENGINE_API makeAbsolutePath( const Path &path );
Path         S2ENGINE_API currentPath();
...
bool         S2ENGINE_API createDir       ( const Path &dirPath );
bool         S2ENGINE_API copyFile        ( const Path &source, const Path &dest );
bool         S2ENGINE_API copyDir         ( const Path &from, const Path& to, bool deleteExisting, const CopyDirOptions &options );
bool         S2ENGINE_API remove          ( const Path &path );
bool         S2ENGINE_API rename          ( const Path &oldPath, const Path &newPath);
bool         S2ENGINE_API exists          ( const Path &file );
uint64_t     S2ENGINE_API available       ( const Path &path, const ByteUnits &unit = ByteUnits::MB );
int64_t      S2ENGINE_API fileSize        ( const Path &path );
...
std::wstring S2ENGINE_API separator       ();
bool         S2ENGINE_API isDirectory     ( const Path &path );
bool         S2ENGINE_API isEmpty         ( const Path &path );
bool         S2ENGINE_API isSymbolicLink  ( const Path &path );
