...
//// ------------------------------------------------------------------------------------------------
//double Viewer::normalizeAngle( const double angle ) 
//{
//	const auto retval = Math::mod( angle, 360.0 );
//	return ( retval > 180.0 ) ? retval-360.0 : retval;
//}
...
bool Viewer2D::fitToBox( const Math::dbox3 &b ) // b is in world space
{
	...
	auto rotBB = b.rotated( _camera.orientation() );
...
Math::dbox3 Viewer2D::rotatedViewBox() const
{
	...
	retval = retval.rotated( _camera.orientation() );
	retval.translate( _camera.position() );
	return retval;
}
...
void Viewer3D::setCameraTopDownAdaptToBox( const Math::dbox3 &b, const BoxAdaptationPolicy adaptPolicy )
{
	...
	_camera = Scene::Camera( cameraPos, target, {0., 1., 0.} );
													   ...
													   const BoxAdaptationPolicy adaptPolicy ) const
{
	...
	return retval.translated( bCenter );
}
