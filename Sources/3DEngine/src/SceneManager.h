// SceneManager.h
//
#ifndef SCENEMANAGER_ONCE
#define SCENEMANAGER_ONCE
...
class S2SCENE_API SceneManager : public Observer, public Observable, public std::enable_shared_from_this<SceneManager>
{
private:
	...
	{
		Renderer::Picker::Value	 rangeBegin;
		Renderer::Picker::Value	 rangeEnd;
	...
	struct MouseEventObj
	{
		...
		void                    clearObj();
		Renderer::Picker::Value pickValue() const;
		int                     pickPart() const;
...
public:
	...
	virtual void             setSurface( const Renderer::SurfacePtr &surface );
	virtual void             onDrawBegin( const View &view ); // tmp eventi di notifica. il disegno andrebbe fatto qui dentro
	virtual void             onDrawEnd();   // tmp eventi di notifica. il disegno andrebbe fatto qui dentro
	Renderer::Picker::Value  negotiatePickValues( const ObjectInteractionPtr &object, const Renderer::Picker::Value = 1 );
