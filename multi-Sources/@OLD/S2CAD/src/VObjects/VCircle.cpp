...
// ------------------------------------------------------------------------------------------------
std::vector<Math::dvec3> VCircle::points() const
{
	std::vector<Math::dvec3> vp(5);
	vp[0] = _center;
	vp[1] = _center + Math::dvec3(_radius, 0, 0);
	vp[2] = _center - Math::dvec3(_radius, 0, 0);
	vp[3] = _center + Math::dvec3(0,_radius,  0);
	vp[4] = _center - Math::dvec3(0,_radius,  0);
	return vp;
}

// ------------------------------------------------------------------------------------------------
VObjectPtr VCircle::clone() const
{
	return New( _center, _radius );
}