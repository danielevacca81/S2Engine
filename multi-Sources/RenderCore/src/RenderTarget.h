...
// You'd need a context object to create a RenderTarget from
class RENDERCORE_API RenderTarget
{
public:
...
public:
	RenderTarget();
	RenderTarget( const std::initializer_list<AttachmentDescription>& init );
	...
	void readPixels( const FrameBuffer::AttachmentPoint &attachPoint, const ImageFormat &pixelFormat, const Math::irect &roi, float *pixels ) const; // This is a convenience function
	void readPixels( const FrameBuffer::AttachmentPoint &attachPoint, const ImageFormat &pixelFormat, const Math::irect &roi, uint8_t *pixels ) const; // This is a convenience function
	void readPixels( const FrameBuffer::AttachmentPoint &attachPoint, const ImageFormat &pixelFormat, const ImageDataType &pixelType, const Math::irect &roi, void *pixels );

	const FrameBufferPtr &fbo() const { return _fbo; }  // WARNING: to be used under special conditions only or for debugging purpose. 
														// It is preferable to use RenderTarget methods to manipulate the underlying fbo
