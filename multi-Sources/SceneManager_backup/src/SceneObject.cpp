...
// ------------------------------------------------------------------------------------------------
void SceneObject::prepareForPicking( RenderSystem::Surface &surface, RenderSystem::DrawState &ds ) const
{
	surface.setPickingEnabled( _pickerInteraction == PickerInteraction::Write );
	if( _pickerInteraction == PickerInteraction::Write )
		ds.shaderProgram->uniform<int>( "u_pickValueOrigin" )->set( RenderSystem::Picker::kInvalidValueBegin );
}
...
*/
void SceneObject::draw( RenderSystem::Surface &surface, const RenderSystem::DrawState &ds ) const
{ 
	if( !_visible )
		return;

	RenderSystem::DrawState localDs = ds;
